...
$(function() {
	$("#treeOrz").tree({
		...
		onSelect:function(node){
			//重新加载数据
			orgid = node.id;
			$('#dg').datagrid({url:ctx+'/sharerole/list/'+node.id});
		}
	});
});
...
function loadSubsystem(){
	$('#subData').datagrid({url:ctx+'/subsystem/sharelists/'+orgid,queryParams: {srid:$('#roleid').val()},"onLoadSuccess":function(data){
    ...
    <div data-options="region:'center'">
		...
		iconCls:'icon-save',resizable:true,modal:true,buttons: [{text:'提交', iconCls:'icon-ok', handler:saveOrUpdate}]">
			...
			<div style="width: 20%;height:100%;float: left;">
			    ...
			    striped="true" url="" data-options="method:'post',selectOnCheck:false,singleSelect:true,onSelect:loadAuthc,onUncheck:emptyMenu"> 
					<thead>
						<tr>
			            	<th field="id" width="50px" checkbox="true">编号</th>
